...
const App = () => {
  ...
  const initialFormState = { id: null, name: '', researcharea: '', maxpostgraduates: '', maxundergraduates: '' }
  	...
  	/*Setting state
    With Hook state, there’s a getter and setter for each type of state (you can have as many as you want)*/
  	const [ advisors, setAdvisors ] = useState(advisorsData)
    /*For editing, when the edit action is selected for an advisor, the Add advisor form will become an
    Edit advisor form, and it will be pre-populated with the data from the selected advisor. The user can
    either cancel edit mode, or submit the change, which will update the selected advisor and close out of
    edit mode. This is done by having a state for whether or not edit mode is turned on. It will begin as false.*/
  	const [ editing, setEditing ] = useState(false)
    /*Since it is not known who is being edited until it’s selected, the state for the form is set to empty and
    assigned to the currentAdvisor so that it can be tracked.*/
    ...
    I incremented the ID of the new advisor manually. This function will take an advisor
    object as a parameter, and add them to the advisors array of objects. The ...advisors
    code ensures that all the previous advisors remain in the array.*/
  	const addAdvisor = advisor => {
  		advisor.id = advisors.length + 1
  		setAdvisors([ ...advisors, advisor ])
  	}

    /*The delete function simply takes the ID of the advisor and filters them out of the advisors array.*/
  	const deleteAdvisor = id => {
  		setEditing(false)
  		setAdvisors(advisors.filter(advisor => advisor.id !== id))
  	}

    /*When the Edit button is selected on an advisor, it passes the current advisor into state to the
    function below which turns on edit mode, and sets the current advisor.*/
    ...
    /*The update function maps through the array, and updates the advisor that matches the ID passed through.*/
